...
class CLIP(BaseModel):
    ...
    def optimize(self, onnx_graph, minimal_optimization=False):
        ...
        bLayerNormPlugin = enable_optimization and self.optimization_level > 0
...
class UNet(BaseModel):
    def get_model(self):
        ...
        return UNet2DConditionModel.from_pretrained("runwayml/stable-diffusion-v1-5",
            subfolder="unet",
    ...
    def optimize(self, onnx_graph, minimal_optimization=False):
        ...
        bMHAPlugin = self.optimization_level > 3
        # Use multi-head cross attention Plugin
        bMHCAPlugin = self.optimization_level > 4
        # Insert GroupNormalization Plugin
        bGroupNormPlugin = self.optimization_level > 1
        # Insert LayerNormalization Plugin
        bLayerNormPlugin = self.optimization_level > 0 
        # Insert Split+GeLU Plugin
        bSplitGeLUPlugin = self.optimization_level > 2
...
class VAE(BaseModel):
    ...
    def optimize(self, onnx_graph, minimal_optimization=False):
        ...
        bRemoveCastNode = enable_optimization
        # Insert GroupNormalization Plugin
        bGroupNormPlugin = enable_optimization and self.optimization_level > 1 
