        ...
        for model_name, obj in self.models.items():
            ...
            if force_build or not os.path.exists(engine.engine_path):
                ...
                if force_optimize or not os.path.exists(onnx_opt_path):
                    obj.optimization_level =  self.optimization_level
            ...
            if not warmup:
                ...
                if save_to_json:
                    if not os.path.isdir("results"):
                        os.mkdir("results")
                    device_prop = cudart.cudaGetDeviceProperties(0)

                    profiling_dict = {
                        "device name":f"{device_prop[1].name}",
                        "CLIP": f"{cudart.cudaEventElapsedTime(events['clip-start'], events['clip-stop'])[1]:>9.2f} ms",
                        "Unet x 50": f"{cudart.cudaEventElapsedTime(events['denoise-start'], events['denoise-stop'])[1]} ms",
                        "VAE":f"{cudart.cudaEventElapsedTime(events['vae-start'], events['vae-stop'])[1]} ms",
                        "Pipeline":f"{(e2e_toc - e2e_tic)*1000.} ms"
                    }

                    with open(f"results/{device_prop[1].name}_op_level_{self.optimization_level}.json", "w") as json_file:
                        json.dump(profiling_dict, json_file)
